    ...
    <div>
      ...
      <Reveal delay={0.1}>
        <div className="glass-form p-6 mb-6">
          ...
          <h4 className="text-[10px] tracking-[0.2em] uppercase text-[#a3a3a3] mb-2">
            {rv.corridorsTitle}
          </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {taxModel.corridors.map((corridor) => (
              <div
                key={corridor.name}
                className="glass-form p-4 border-l-2 border-[#C5A059] hover:border-[#DFBD69] transition-colors"
              >
                <div className="flex items-center gap-2 mb-2">
                  <Globe className="w-3 h-3 text-[#C5A059]" />
                  <span className="text-[10px] text-[#a3a3a3]">{corridor.name}</span>
                </div>
                <p className="font-mono text-lg text-[#ffffff] font-bold">
            ...
            {flowSteps.map((step, i) => (
              <div key={step.label} className="flex items-start">
                <div className="flex flex-col items-center text-center flex-1 min-w-0">
                  <div className="w-14 h-14 rounded-xl glass-form flex items-center justify-center mb-3 border border-[rgba(197,160,89,0.3)]">
                    <step.icon className="w-6 h-6 text-[#C5A059]" />
                  </div>
                  <span className="text-[9px] text-[#C5A059] font-mono mb-1">{step.month}</span>
                  <p className="text-xs font-semibold text-[#ffffff] mb-1 leading-tight">
                    {step.label}
                  </p>
                  <p className="text-[10px] text-[#a3a3a3] leading-relaxed max-w-[140px]">
                    {step.description}
                  </p>
                </div>
                {i < flowSteps.length - 1 && (
                  <ArrowRight className="w-5 h-5 text-[#C5A059] mt-4 mx-1 shrink-0" />
                )}
              </div>
            ))}
          </div>
            ...
            {flowSteps.map((step, i) => (
              <div key={step.label}>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl glass-form flex items-center justify-center shrink-0 border border-[rgba(197,160,89,0.3)]">
                    <step.icon className="w-5 h-5 text-[#C5A059]" />
                  </div>
                  <div>
                    <span className="text-[9px] text-[#C5A059] font-mono">{step.month}</span>
                ...
                {i < flowSteps.length - 1 && (
                  <div className="ml-6 h-6 border-l border-dashed border-[rgba(197,160,89,0.3)]" />
                )}
              </div>
            ))}
          </div>
      ...
      <Reveal delay={0.4}>
        <div className="glass-form p-6">
          ...
          <p className="text-[10px] tracking-[0.2em] uppercase text-[#a3a3a3] mb-6">
            {rv.documentRoomSub}
          </p>
          ...
          <div className="flex flex-col gap-3">
            {documents.map((doc) => (
              <div
                key={doc.file}
                className="flex items-center justify-between p-4 rounded-lg bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] hover:border-[rgba(197,160,89,0.2)] transition-all duration-300 group"
              >
                <div className="flex items-center gap-3">
                  <FileText className="w-5 h-5 text-[#EF4444] shrink-0" />
                  <div>
                    <p className="text-sm text-[#ffffff] group-hover:text-[#C5A059] transition-colors">
                    ...
                    <p className="text-[10px] text-[#a3a3a3] leading-relaxed max-w-sm">
                      {doc.desc}
                    </p>
                  </div>
                </div>
                <button className="btn-gold flex items-center gap-2 px-4 py-2 text-[10px] shrink-0">
