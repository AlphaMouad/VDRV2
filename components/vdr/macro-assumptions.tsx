      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {sublabel && (
              <p className="text-[10px] text-[#a3a3a3]">{sublabel}</p>
            )}
          </div>
        </div>
      ...
      <Reveal delay={0.1}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="glass-form p-4">
            <p className="text-[10px] tracking-[0.2em] uppercase text-[#a3a3a3] mb-1">{t.macroView.totalGdv}</p>
            <p className="gold-text-gradient font-mono text-xl font-bold">
              €{totalGDV.toLocaleString()}
            </p>
          </div>
          <div className="glass-form p-4">
            <p className="text-[10px] tracking-[0.2em] uppercase text-[#a3a3a3] mb-1">{t.macroView.totalGdc}</p>
            <p className="font-mono text-xl text-[#ffffff] font-bold">
              €{totalGDC.toLocaleString()}
            </p>
          </div>
          <div className="glass-form p-4">
            <p className="text-[10px] tracking-[0.2em] uppercase text-[#a3a3a3] mb-1">{t.macroView.grossMarkup}</p>
            <p className="font-mono text-xl text-[#10B981] font-bold">{grossMargin}%</p>
          </div>
          <div className="glass-form p-4">
            <p className="text-[10px] tracking-[0.2em] uppercase text-[#a3a3a3] mb-1">{t.macroView.vefaSum}</p>
            <p className={`font-mono text-xl font-bold ${vefaTotal === 100 ? "text-[#10B981]" : "text-[#EF4444]"}`}>
              {vefaTotal}%
            </p>
            {vefaTotal !== 100 && (
              <p className="text-[10px] text-[#EF4444] mt-0.5">{t.macroView.vefaMustTotal}</p>
            )}
          </div>
        </div>
      </Reveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="glass-form p-4 border border-[rgba(197,160,89,0.1)]">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] tracking-[0.15em] uppercase text-[#a3a3a3]">{t.macroView.totalVillas}</p>
              </div>
              <p className="gold-text-gradient font-mono text-2xl font-bold">{macro.totalVillas}</p>
              <p className="text-[9px] text-[#a3a3a3] mt-1">{t.macroView.totalVillasSub}</p>
            </div>
            <div className="glass-form p-4 border border-[rgba(197,160,89,0.1)]">
              <div className="flex items-center gap-2 mb-2">
                <Banknote className="w-3.5 h-3.5 text-[#C5A059]" />
                <p className="text-[10px] tracking-[0.15em] uppercase text-[#a3a3a3]">{t.macroView.gdcPerVilla}</p>
              </div>
              <p className="gold-text-gradient font-mono text-2xl font-bold">€{macro.gdcPerVilla.toLocaleString()}</p>
              <p className="text-[9px] text-[#a3a3a3] mt-1">{t.macroView.gdcPerVillaSub}</p>
            </div>
            <div className="glass-form p-4 border border-[rgba(197,160,89,0.1)]">
              <div className="flex items-center gap-2 mb-2">
                <Banknote className="w-3.5 h-3.5 text-[#10B981]" />
                <p className="text-[10px] tracking-[0.15em] uppercase text-[#a3a3a3]">{t.macroView.avgVillaGdv}</p>
              </div>
              <p className="font-mono text-2xl font-bold text-[#10B981]">€{macro.avgVillaGDV.toLocaleString()}</p>
              <p className="text-[9px] text-[#a3a3a3] mt-1">{t.macroView.avgVillaGdvSub}</p>
            </div>
          </div>
        </div>
      </Reveal>
